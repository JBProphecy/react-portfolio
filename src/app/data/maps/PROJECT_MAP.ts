...
export const PROJECT_MAP: Record<ProjectKey, ProjectData> = {
  ...
  [ProjectKey.MyPortfolio]: {
    ...
    description: "This project is my portfolio. It's a purely front-end, single-page application built with React. I've designed it to be responsive and transition seamlessly between various screen sizes using custom scaling units and media queries. While I've used some custom scaling units, I made the font-size scale work in relation to rem, so that way a user's font settings can still apply. I've also made every interaction interact with the URL so that way the section, modal, and sidebar can all be navigated back and forth with the browser controls. I have some information about me, links to my resume and socials, and information about some of my projects with links to their repositories, so feel free to check them out.",
    repositoryLinkText: "Open Repository in a New Tab",
